...
const ResourceState = (props) => {
  ...
  const sortResources = (resources) => {
    return resources.sort((a, b) => ('' + a.repoName).localeCompare(b.repoName));
  };
  ...
  const getIntialResources = () => {
    ...
    else if (pathname === '/bookmarked') return sortResources(localStorageResources);
  };
  ...
  const getIntialFilteredResources = () => {
    ...
    else if (searchText.trim() !== '') return filterResourcesOnChangeInSearchText(searchText);
