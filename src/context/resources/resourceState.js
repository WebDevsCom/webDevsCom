...
import {
  SET_FILTERED_RESOURCES, SET_PAGE_TITLE
} from '../types';

const ResourceState = props => {
  const intialState = { resources: [], category: "all", searchText: "", pageTitle: "" };
  ...
  const isPresent = (value, checkValue) => value.toLowerCase().includes(checkValue);
  ...
  const getIntialFilteredResources = () => {
    ...
    if (category !== "all")
      return filterResourcesOnChangeInCategory(category);
    else if (searchText.trim() !== "")
      return filterResourcesOnChangeInSearchText(searchText);
    else
      return getIntialResources();
  }
  ...
  const filterResourcesOnChangeInSearchText = (val) => {
    ...
    filteredResources =
      resources.filter(
        ({ repoOwner, repoOwnerName, description, repoName }) => isPresent(repoName, searchInput) || isPresent(repoOwner, searchInput) || isPresent(description, searchInput) || isPresent(repoOwnerName, searchInput)
      );
  ...
  const setCategory = (val) => {
    dispatch({
      ...
      payload: {
        category: val, searchText: "", resources: filterResourcesOnChangeInCategory(val)
      }
    })
  }
  ...
  const setSearchText = (val) => {
    dispatch({
      type: SET_FILTERED_RESOURCES,
      payload: {
        category: "all", searchText: val, resources: filterResourcesOnChangeInSearchText(val)
      }
    })
  }
  ...
  useEffect(() => {
    const { pathname } = window.location;
    if (pathname !== "/resources" && pathname !== "/bookmarked")
      return;
    dispatch({
      type: SET_FILTERED_RESOURCES,
      payload: {
